...
export default function Home() {
  ...
  const results = useMemo(() => {
    // -1 significa: no hay día que coincida con la búsqueda => sin resultados
    if (filterDay === -1) return { alternatives: [] };
    return computeResults(filterDay ?? undefined);
  }, [filterDay, computeResults]);

  // ✅ Mantener selección válida siempre
  useEffect(() => {
    if (!state?.people?.length) return;

    const stillExists =
      selectedPersonId && state.people.some((p) => p.id === selectedPersonId);
  ...
  useEffect(() => {
    ...
    // si está vacío: muestra todos los días
    if (!q) {
      setFilterDay(null);
      return;
    }

    // match por includes: "lu" => "Lunes"
    const matchedIndex = DAYS_NAMES.findIndex((d) =>
      d.toLowerCase().includes(q)
    );

    // si hay match, filtra ese día; si no, marca "sin resultados"
  ...
  const handleAddSlot = () => {
    ...
    if (to <= from) {
      toast({
        title: "Error",
        description: "El fin debe ser después del inicio.",
        variant: "destructive",
      });
      return;
    }

    addSlot(selectedPersonId, parseInt(selectedDay, 10), from, to);
    toast({
      title: "Horario bloqueado",
      description: "El bloqueo se ha registrado correctamente.",
    });
  };
  ...
  const handleBlockFullDay = () => {
    ...
    addSlot(selectedPersonId, parseInt(selectedDay, 10), 0, 1439);
    toast({
      title: "Día Bloqueado",
      description: "Se ha marcado el día completo como ocupado.",
    });
  };
  ...
  const handleAddPersonSubmit = () => {
    ...
    toast({
      title: "Amigo añadido",
      description: "Se ha agregado a la lista correctamente.",
    });
  };
  ...
  const handleEditPersonSubmit = () => {
    ...
    toast({
      ...
      description: "Se ha guardado el cambio correctamente.",
    });
  };
  ...
  const handleResetConfirm = () => {
    ...
    // forzar que no quede pegado un id viejo
    setSelectedPersonId("");
    ...
    toast({
      ...
      description: "Se eliminó todo y quedó solo 'Yo'.",
    });
  };
        ...
        <header className="flex flex-col md:flex-row justify-between items-end gap-6 border-b border-border pb-8">
          <div className="space-y-3">
            ...
            <h1 className="text-5xl font-headline font-bold tracking-tight">
              Coordinador de juntitas 
            </h1>
            <p className="text-muted-foreground text-lg max-w-xl">
              El que no apaña al inser es wekito{" "}
              ...
              <div className="lg:col-span-4 space-y-6">
                <Card className="border-border shadow-xl overflow-hidden">
                  <CardHeader className="bg-muted/30">
                    ...
                    <CardDescription>
                      Indica cuándo NO están disponibles.
                    </CardDescription>
                  </CardHeader>
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    {state.people.map((p, idx) => (
                      <div
                        key={p.id}
                        className="flex items-center justify-between p-3 rounded-lg bg-card border border-border group"
                      >
                        ...
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          ...
                          {idx !== 0 && (
                            <Button
                              ...
                              onClick={() => {
                                removePerson(p.id);
                                if (selectedPersonId === p.id) setSelectedPersonId("");
                              }}
                ...
                <Card className="border-border shadow-xl h-full">
                  <CardHeader className="flex flex-row items-center justify-between space-y-0">
                    <div>
                      ...
                      <CardDescription>
                        Vista semanal de la persona seleccionada.
                      </CardDescription>
                    </div>
                  ...
                  <CardContent>
                    <div className="grid grid-cols-1 sm:grid-cols-7 gap-2">
                      {DAYS_NAMES.map((name, i) => {
                        const slots = selectedPersonId
                          ? state.availability[selectedPersonId]?.[i] || []
                          : [];
                          ...
                          <div key={i} className="space-y-2">
                            ...
                            <div className="min-h-[300px] bg-muted/20 rounded-lg p-1 space-y-1">
                              {slots.map((s, idx) => (
                                <div
                                  key={idx}
                                  className="relative group/block bg-destructive/10 text-destructive border border-destructive/20 rounded-md p-2 text-[10px] font-bold"
                                >
                                  {fromMin(s.fromMin)} - {fromMin(s.toMin)}
                                  <button
                                    onClick={() => removeSlot(selectedPersonId, i, idx)}
                                    className="absolute -top-1 -right-1 bg-destructive text-white rounded-full p-0.5 opacity-0 group-hover/block:opacity-100"
                              ...
                              {slots.length === 0 && (
                                <div className="h-full flex items-center justify-center opacity-20 italic text-[10px] py-10">
                                  Libre
                                </div>
                              )}
                            </div>
              ...
              <div className="lg:col-span-3 space-y-6">
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    <div className="relative">
                      <Input
                        placeholder="Buscar día... (ej: lunes, mar)"
                        value={daySearch}
                    ...
                    {filterDay === -1 && (
                      <div className="text-xs text-muted-foreground italic px-2 py-3">
                        No se encontró ningún día.
                      </div>
                    )}

                    {/* opcional: hint cuando sí hay match */}
                    {filterDay !== null && filterDay !== -1 && (
                      <div className="text-xs text-muted-foreground px-2">
                        Mostrando: <span className="font-semibold">{DAYS_NAMES[filterDay]}</span>
                        <Button
                          variant="ghost"
                          size="sm"
                ...
                <div className="p-4 rounded-xl bg-primary/5 border border-primary/20 space-y-3">
                  ...
                  <p className="text-xs text-muted-foreground leading-relaxed">
                    Estos son los momentos donde <strong>todos</strong> están libres
                    simultáneamente.
                  </p>
                </div>
              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {results.alternatives.length > 0 ? (
                    results.alternatives.map((alt, i) => (
                      <Card
                        key={i}
                        className="group hover:border-primary/50 transition-all duration-300 border-primary/40 bg-primary/5 shadow-md"
                      >
                              ...
                              <div className="flex justify-between items-start">
                                <div className="space-y-1">
                                  ...
                                  <p className="text-xs text-muted-foreground">
                                    Ventana de {((alt.end - alt.start) / 60).toFixed(1)}{" "}
                                    horas
                                  </p>
                                </div>
                              ...
                              <div className="flex flex-wrap gap-1.5 pt-2 border-t border-primary/10">
                                {alt.can.map((p) => (
                                  <Badge
                                    key={p.id}
                                    variant="outline"
                                    className="text-[10px] font-medium border-primary/20 bg-background/50"
                                  >
                                    {p.name}
                                  </Badge>
                                ))}
                    ...
                    <div className="col-span-full py-20 text-center space-y-4 border-2 border-dashed border-border rounded-3xl bg-muted/10">
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-muted-foreground max-w-xs mx-auto text-sm">
                          Parece que no hay un horario donde todos estén libres. Prueba a
                          eliminar algún bloqueo.
                        </p>
                      </div>
                    </div>
                  )}
      ...
      <Dialog open={isResetModalOpen} onOpenChange={setIsResetModalOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Esto eliminará <strong>todos los bloqueos</strong> y{" "}
              <strong>todos los amigos</strong>. Quedará solo{" "}
              <strong>“Yo”</strong>. Esta acción no se puede deshacer.
            </DialogDescription>
          </DialogHeader>

          <div className="rounded-lg border border-destructive/20 bg-destructive/5 p-3 text-sm text-muted-foreground">
            Tip: si solo quieres corregir un horario, puedes borrar un bloqueo
            específico desde el Resumen.
          </div>
